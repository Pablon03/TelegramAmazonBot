...
"""
Un array con {CATEGORY_NAME: [<LIST OF THE CATEGORY KEYWORDS>]}
"""
...
def run_bot(bot: telegram.Bot, categories: Dict[str, List[str]]) -> None:
    ...
    while True:
        try:
            ...
            logging.info(f'{5 * "*"} Consulta completada {5 * "*"}')
            ...
            while len(res) > 3:
                ...
                if is_active():
                    try:
                        ...
                        logging.info(f'{5 * "*"} Enviando publicaciones al canal {5 * "*"}')
                    ...
                    # Descansa por PAUSE_MINUTES
                    time.sleep(60 * PAUSE_MINUTES)

                else:
                    # Si el bot no está activo
                    logging.info(
                        f'{5 * "*"} Bot Inactivo, entre  {MIN_HOUR}AM y {MAX_HOUR}PM {5 * "*"}'
                    )
                    time.sleep(60 * 5)
